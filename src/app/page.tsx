...
export default function Dashboard() {
  ...
  const filteredVoyages = voyages.filter(v => {
    ...
    if (viewMode === "unrated") {
      return v.bls.some((bl: any) => bl.statutFret?.toLowerCase() === "unrated" || bl.statut?.toLowerCase() === "unrated");
    }
    if (viewMode === "no-freight") {
      return v.bls.some((bl: any) => (!bl.statutFret || bl.statutFret.trim() === "") && bl.statut?.toLowerCase() !== "unrated");
    }
  ...
  const totalUnratedBLs = voyages.reduce((acc, v) => acc + v.bls.filter((bl: any) => bl.statutFret?.toLowerCase() === "unrated" || bl.statut?.toLowerCase() === "unrated").length, 0);
  const totalNoFreightBLs = voyages.reduce((acc, v) => acc + v.bls.filter((bl: any) => (!bl.statutFret || bl.statutFret.trim() === "") && bl.statut?.toLowerCase() !== "unrated").length, 0);
        ...
        <div className="space-y-12">
          ...
          <section>
            <div className="flex items-center gap-3 mb-6 pb-2 border-b border-slate-100">
              ...
              <h2 className="text-2xl font-black text-slate-700 tracking-tight">Vue Globale</h2>
            </div>
            ...
            <div className="space-y-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
                ...
                >
                  <p className="text-slate-700 font-bold uppercase tracking-widest text-[10px] h-8">Total Non Relâchés</p>
                  <div className="flex items-center gap-4">
                    <div className="bg-purple-50 p-2.5 rounded-2xl flex-shrink-0 flex items-center justify-center">
                      <AlertCircle className="w-5 h-5 text-purple-500" />
                    </div>
                    <p className="text-4xl font-black text-purple-600 leading-none">{totalUnreleasedBLs}</p>
                  </div>
                </button>
                ...
                >
                  <p className="text-slate-700 font-bold uppercase tracking-widest text-[10px] h-8">Non Relâchés &gt; 15J</p>
                  <div className="flex items-center gap-4">
                    <div className="bg-orange-50 p-2.5 rounded-2xl flex-shrink-0 flex items-center justify-center">
                      <Clock className="w-5 h-5 text-orange-500" />
                    </div>
                    <p className="text-4xl font-black text-orange-600 leading-none">{totalCriticalBLs}</p>
                  </div>
                </button>
                ...
                >
                  <p className="text-slate-700 font-bold uppercase tracking-widest text-[10px] h-8">Dossiers Non Scannés</p>
                  <div className="flex items-center gap-4">
                    <div className="bg-rose-50 p-2.5 rounded-2xl flex-shrink-0 flex items-center justify-center">
                      <CloudUpload className="w-5 h-5 text-rose-500" />
                    </div>
                    <p className="text-4xl font-black text-rose-600 leading-none">{totalNoScannedBLs}</p>
                  </div>
                </button>
                ...
                >
                  <p className="text-slate-700 font-bold uppercase tracking-widest text-[10px] h-8">Notes Internes</p>
                  <div className="flex items-center gap-4">
                    <div className="bg-indigo-50 p-2.5 rounded-2xl flex-shrink-0 flex items-center justify-center">
                      <MessageSquare className="w-5 h-5 text-indigo-500" />
                    </div>
                    <p className="text-4xl font-black text-indigo-600 leading-none">{totalWithNotesBLs}</p>
                  </div>
                </button>
                ...
                <button 
                  ...
                  className={`p-5 rounded-3xl border transition-all flex flex-col gap-3 text-left cursor-pointer ${
                    (viewMode as string) === "unrated" ? "bg-slate-50 border-blue-300 ring-2 ring-blue-100" : "bg-white border-blue-100 shadow-xl shadow-blue-500/5 hover:border-blue-300"
                  }`}
                >
                  <p className="text-slate-700 font-bold uppercase tracking-widest text-[10px] h-8">Statut Fret Unrated</p>
                  <div className="flex items-center gap-4">
                    <div className="bg-slate-50 p-2.5 rounded-2xl flex-shrink-0 flex items-center justify-center">
                      <AlertTriangle className="w-5 h-5 text-blue-500" />
                    </div>
                    <p className="text-4xl font-black text-blue-400 leading-none">{totalUnratedBLs}</p>
                  </div>
                </button>
                ...
                <button 
                  ...
                  className={`p-5 rounded-3xl border transition-all flex flex-col gap-3 text-left cursor-pointer ${
                    (viewMode as string) === "no-freight" ? "bg-brand-bg border-slate-200 ring-2 ring-slate-100" : "bg-white border-slate-100 shadow-xl shadow-slate-500/5 hover:border-slate-200"
                  }`}
                >
                  <p className="text-slate-700 font-bold uppercase tracking-widest text-[10px] h-8">Pas Encore Freté</p>
                  <div className="flex items-center gap-4">
                    <div className="bg-brand-bg p-2.5 rounded-2xl flex-shrink-0 flex items-center justify-center">
                      <CircleSlash className="w-5 h-5 text-slate-700-muted" />
                    </div>
                    <p className="text-4xl font-black text-slate-700-dim leading-none">{totalNoFreightBLs}</p>
                  </div>
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6 max-w-4xl">
                <div className="bg-white p-6 rounded-3xl border border-blue-100 shadow-xl shadow-blue-500/5 flex flex-col gap-4">
                  <p className="text-slate-700 font-bold uppercase tracking-widest text-[11px]">Total de BLs</p>
                  <div className="flex items-center gap-5">
                    <div className="bg-slate-50 p-3 rounded-2xl flex-shrink-0 flex items-center justify-center">
                      <FileSpreadsheet className="w-6 h-6 text-blue-500" />
                    </div>
                    <p className="text-4xl md:text-5xl font-black text-blue-400 leading-none">{totalBLs}</p>
                  </div>
                </div>

                <div className="bg-white p-6 rounded-3xl border border-teal-100 shadow-xl shadow-teal-500/5 flex flex-col gap-4">
                  <p className="text-slate-700 font-bold uppercase tracking-widest text-[11px]">Total de Voyages</p>
                  <div className="flex items-center gap-5">
                    <div className="bg-teal-50 p-3 rounded-2xl flex-shrink-0 flex items-center justify-center">
                      <ShipWheel className="w-6 h-6 text-teal-500" />
                    </div>
                    <p className="text-4xl md:text-5xl font-black text-teal-600 leading-none">{totalVoyages}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-10">
          {viewMode !== "default" && (
            <div className={`p-4 rounded-2xl flex justify-between items-center ${
              ...
              viewMode === "unrated" ? "bg-slate-50 text-blue-400" :
              viewMode === "no-freight" ? "bg-brand-bg text-slate-700" :
          ...
          {viewMode !== "default" ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {flatBLs.length > 0 ? flatBLs.map(({bl, voyage, daysSinceETD}) => (
                <div 
                  ...
                  className={`bg-white rounded-3xl p-6 border border-2 border-transparent shadow-xl shadow-black/50/50 transition-all cursor-pointer group relative overflow-hidden flex flex-col ${
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1 px-1">
                        <span className="text-[10px] font-bold text-slate-700 max-w-[150px] truncate" title={bl.shipper}>{bl.shipper || "Sans chargeur"}</span>
                      </div>
                    </div>
                    <div className={`${
                      viewMode === 'critical' ? 'bg-orange-50 group-hover:bg-orange-500' : 
                      viewMode === 'unrated' ? 'bg-slate-50 group-hover:bg-blue-500' : 
                  ...
                  <div className="space-y-1.5 mb-4 flex-1">
                    <div className="flex items-center gap-2 text-[11px] text-slate-700">
                      ...
                      <span className="font-mono text-[10px] bg-slate-50 px-1.5 py-0.5 rounded-md text-slate-700 font-bold">{voyage.numero}</span>
                    </div>
                    <div className="flex items-center gap-2 text-[11px] text-slate-700">
                    ...
                    {bl.commentaire && (
                      <div className="mt-4 pt-3 border-t border-gray-50">
                        <div className="flex items-center gap-1.5 mb-1.5">
                           <MessageSquare className="w-3 h-3 text-blue-400 opacity-70" />
                           <span className="text-[9px] font-black text-slate-700-muted uppercase tracking-widest">Note Interne</span>
                        </div>
            ...
            filteredVoyages.length > 0 ? (
              ...
              <div className="glass rounded-3xl p-20 flex flex-col items-center justify-center text-center border-dashed border-2 border-slate-200">
